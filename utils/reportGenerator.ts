...
export const generatePerformanceSvg = (history: TrainingMetricPoint[], result?: DetectionResult | null): string => {
  ...
  return `
    <svg width="100%" height="200" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <line x1="${padding}" y1="${height - padding}" x2="${width - padding}" y2="${height - padding}" stroke="#111" />
      <line x1="${padding}" y1="${padding}" x2="${padding}" y2="${height - padding}" stroke="#111" />
      <path d="${areaPath}" fill="${COLORS.primary}" fill-opacity="0.1" stroke="${COLORS.primary}" stroke-width="2" />
      ${missionRefLine}
      <text x="${padding}" y="${height - 10}" fill="#334155" font-size="10">Training Start</text>
      <text x="${width - padding - 60}" y="${height - 10}" fill="#334155" font-size="10">Latest Epoch</text>
    </svg>
  `;
};
...
export const generateMetricsBarSvg = (result: DetectionResult): string => {
  ...
  const metrics = [
    ...
    { label: 'Segmentation', val: result.technicalDetails.segmentationFidelity, color: COLORS.danger }
  ];
  ...
  metrics.forEach((m, i) => {
    ...
    bars += `
      ...
      <text x="${padding + labelWidth + barW + 10}" y="${y + 18}" fill="#F1F5F9" font-size="12" font-weight="bold">${(m.val * 100).toFixed(1)}%</text>
    `;
  });
...
export const generateRadarSvg = (metrics: MetricPoint[]): string => {
  ...
  [0.2, 0.4, 0.6, 0.8, 1].forEach(r => {
    ...
    grid += `<path d="${path} Z" fill="none" stroke="#111" stroke-width="1" />`;
  });
  ...
  return `
    <svg width="100%" height="300" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
      ...
      ${metrics.map((m, i) => {
        ...
        return `<text x="${x}" y="${y}" fill="#334155" font-size="9" text-anchor="middle" font-weight="bold">${m.subject.toUpperCase()}</text>`;
      }).join('')}
    </svg>
  `;
};
