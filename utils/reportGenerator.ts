...
export const generatePerformanceSvg = (history: TrainingMetricPoint[], result?: DetectionResult | null): string => {
  ...
  return `
    <svg width="100%" height="200" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <line x1="${padding}" y1="${height - padding}" x2="${width - padding}" y2="${height - padding}" stroke="#1a1a1a" />
      <line x1="${padding}" y1="${padding}" x2="${padding}" y2="${height - padding}" stroke="#1a1a1a" />
...
export const generateMetricsBarSvg = (result: DetectionResult): string => {
  ...
  const metrics = [
    ...
    { label: 'Segmentation', val: result.technicalDetails.segmentationFidelity, color: COLORS.info }
  ];
  ...
  metrics.forEach((m, i) => {
    ...
    bars += `
      ...
      <rect x="${padding + labelWidth}" y="${y}" width="${width - padding * 2 - labelWidth}" height="25" fill="#0A192F" rx="4" />
      <rect x="${padding + labelWidth}" y="${y}" width="${barW}" height="25" fill="${m.color}" rx="4" />
      <text x="${padding + labelWidth + barW + 10}" y="${y + 18}" fill="#FFFFFF" font-size="12" font-weight="bold">${(m.val * 100).toFixed(1)}%</text>
    `;
  });
  ...
  return `
    <svg width="100%" height="300" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
      ...
      ${metrics.map((m, i) => {
        ...
        return `<text x="${x}" y="${y}" fill="#94A3B8" font-size="9" text-anchor="middle" font-weight="bold">${m.subject.toUpperCase()}</text>`;
      }).join('')}
    </svg>
  `;
};
