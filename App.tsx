  ...
  <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center p-8 animate-in fade-in duration-300">
    <div className="w-full max-md text-center px-4">
      <div className="mb-8 relative flex justify-center">
        <div className="w-12 h-12 md:w-16 md:h-16 border border-[#F97316]/20 rounded-full animate-ping absolute"></div>
        <i className="fa-solid fa-satellite-dish text-[#F97316] text-2xl md:text-3xl shadow-[0_0_20px_#F97316]"></i>
      </div>
      <h3 className="text-[10px] md:text-[12px] font-semibold text-[#F97316] font-cinzel uppercase tracking-[0.4em] md:tracking-[0.6em] mb-4">{message}</h3>
      <div className="w-full h-[2px] bg-slate-900 overflow-hidden rounded-full">
          <div className="h-full bg-[#F97316] shadow-[0_0_10px_#F97316] animate-[progress_2s_linear_infinite]"></div>
      </div>
    </div>
  </div>
);
...
const ImagePreviewModal: React.FC<{ src: string; onClose: () => void }> = ({ src, onClose }) => (
  <div className="fixed inset-0 z-[150] bg-black/98 flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
    <button className="absolute top-4 right-4 md:top-8 md:right-8 text-slate-500 hover:text-[#F97316] transition-colors z-[160]" onClick={onClose}>
      <i className="fa-solid fa-xmark text-3xl md:text-4xl"></i>
    </button>
    <img src={src} alt="Preview" className="max-w-full max-h-full object-contain rounded-lg md:rounded-xl border border-white/10 shadow-2xl" onClick={(e) => e.stopPropagation()} />
  </div>
);
...
const App: React.FC = () => {
  ...
  const exportReport = () => {
    ...
    const reportHtml = `
      ...
      <head>
        ...
        <style>
          body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #000000; color: #F1F5F9; padding: 40px; }
          .container { max-width: 900px; margin: 0 auto; border: 1px solid rgba(249, 115, 22, 0.2); padding: 40px; border-radius: 20px; background: #050505; box-shadow: 0 10px 40px rgba(0, 0, 0, 0.8); }
          h1 { color: #F97316; text-transform: uppercase; letter-spacing: 5px; border-bottom: 2px solid #F97316; padding-bottom: 20px; text-align: center; }
          .metrics { display: flex; justify-content: space-between; margin-top: 40px; }
          .metric-box { text-align: center; flex: 1; padding: 20px; border-right: 1px solid #111; }
          .metric-box:last-child { border-right: none; }
          .label { font-size: 10px; color: #94A3B8; text-transform: uppercase; margin-bottom: 10px; letter-spacing: 2px; font-weight: bold; }
          .value { font-size: 32px; font-weight: 800; color: #FFFFFF; }
          .description { margin-top: 40px; background: #0A192F; padding: 30px; border-radius: 10px; line-height: 1.6; font-style: italic; border-left: 5px solid #F97316; border: 1px solid rgba(249, 115, 22, 0.1); }
          .visuals { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 40px; }
          .img-container { text-align: center; background: #000; padding: 10px; border-radius: 10px; border: 1px solid #111; }
          img { width: 100%; border-radius: 10px; border: 1px solid #222; margin-top: 10px; }
          .section-title { margin-top: 60px; font-size: 14px; text-transform: uppercase; letter-spacing: 4px; color: #F97316; border-left: 3px solid #F97316; padding-left: 15px; margin-bottom: 20px; font-weight: 800; }
          .charts-container { display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-top: 30px; }
          .chart-box { background: #000; border: 1px solid #111; padding: 20px; border-radius: 15px; }
          .footer { margin-top: 80px; text-align: center; color: #334155; font-size: 10px; letter-spacing: 5px; text-transform: uppercase; font-weight: bold; }
        </style>
      </head>
      <body>
        <div class="container">
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden flex flex-col bg-black text-slate-100">
      ...
      {(stage === 'dashboard' || stage === 'welcome') && (
        <div className="fixed top-4 left-4 md:top-10 md:left-12 z-50 flex items-center gap-3">
           <h1 onClick={returnHome} className="text-lg md:text-2xl font-cinzel font-black text-[#F97316] cursor-pointer tracking-widest uppercase">OIL DETECTOR</h1>
        </div>
      )}
      ...
      {stage === 'landing' && (
        <div className="flex-1 min-h-screen flex items-center justify-center flex-col px-6 text-center relative z-10 page-transition cursor-pointer" onClick={() => performTransition(() => setStage('welcome'), "AUTHENTICATING")}>
          ...
          <p className="mt-8 text-slate-400 font-lexend text-sm md:text-xl italic tracking-widest uppercase animate-pulse">Silent Guardians of Waves.</p>
        </div>
      )}
      ...
      {stage === 'welcome' && (
        <div className="flex-1 min-h-screen flex items-center justify-center flex-col relative z-10 page-transition">
          <div className="max-w-4xl w-full glass-panel p-12 md:p-24 rounded-[3.5rem] text-center">
            ...
            <p className="text-sm md:text-lg text-slate-400 leading-relaxed mb-16 font-lexend tracking-widest max-w-2xl mx-auto">Organic anomaly tracking via <span className="text-[#F97316] font-bold">Obsidian U-Net</span> synthetic aperture radar.</p>
               ...
               {activeMenu === 'Upload Images' && (
                 <div className="space-y-12">
                   <div className="flex justify-center">
                     <div className="w-full max-w-3xl glass-panel p-12 rounded-[2.5rem] flex flex-col items-center justify-center min-h-[500px] cursor-pointer group hover:border-[#F97316]/40 transition-all" onClick={() => fileInputRef.current?.click()}>
                       {selectedImage ? <img src={selectedImage} className="max-h-[450px] rounded-2xl object-contain shadow-2xl" /> : (<div className="text-center opacity-30 group-hover:opacity-80 transition-opacity"><i className="fa-solid fa-satellite text-7xl mb-8 text-[#F97316]"></i><h3 className="text-lg font-cinzel font-bold text-white uppercase tracking-widest">Input SAR/Drone Image</h3><p className="text-[10px] mt-4 text-slate-500 tracking-widest uppercase">Awaiting Uplink</p></div>)}
                   ...
                   <div className="flex flex-col items-center gap-8">
                     ...
                     <div className="w-full max-w-4xl glass-panel p-6 rounded-2xl bg-black/80 border border-white/5 overflow-hidden font-mono text-[11px]">
                        <div className="flex justify-between items-center mb-4 pb-2 border-b border-white/10 uppercase tracking-widest text-[#F97316] font-bold">Tactical Console</div>
                        <div className="space-y-1.5 h-32 overflow-y-auto custom-scrollbar">{logs.map((log, idx) => (<div key={idx} className="text-orange-400 opacity-80"><span className="text-white/20 mr-2">[{new Date().toLocaleTimeString()}]</span>{log}</div>))}</div>
                     </div>
                   </div>
                 </div>
               )}
               ...
               {activeMenu === 'View Results' && result && result.visuals && (
                 <div className="space-y-10">
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                     <div className={`glass-panel p-10 rounded-[2rem] border-l-4 ${result.spillFound ? 'border-[#F97316]' : 'border-[#4ADE80]'}`}><p className="text-[11px] font-bold text-white/30 uppercase mb-3 tracking-widest">Confidence</p><p className="text-5xl font-cinzel text-white font-black">{(result.confidence * 100).toFixed(0)}%</p></div>
                     <div className="glass-panel p-10 rounded-[2rem] border-l-4 border-slate-700"><p className="text-[11px] font-bold text-white/30 uppercase mb-3 tracking-widest">IoU Fidelity</p><p className="text-5xl font-cinzel text-white font-black">{result.iou.toFixed(3)}</p></div>
                   ...
                   {!result.spillFound && (
                     <div className="glass-panel p-8 rounded-2xl bg-[#4ADE80]/5 border border-[#4ADE80]/20 text-[#4ADE80] flex items-center gap-4">
                   ...
                   <div className="glass-panel p-12 rounded-[3.5rem]">
                      ...
                      <div className="bg-black/40 p-12 rounded-[2.5rem] border border-white/5">
                        <h4 className="text-[12px] font-bold text-[#F97316] uppercase tracking-[0.5em] mb-6">Analytical Briefing</h4>
                        <p className="text-xl text-white/90 font-normal leading-relaxed italic border-l-4 border-[#F97316] pl-10">"{result.description}"</p>
                      </div>
                   </div>
                 </div>
               )}
                 ...
                 <div className="space-y-12">
                   <div className="flex flex-wrap gap-4 mb-8">
                      {CHART_TYPES.map(type => (
                        <button key={type} onClick={() => toggleChart(type)} className={`px-6 py-2 rounded-full text-[10px] uppercase tracking-widest transition-all ${activeCharts.includes(type) ? 'bg-[#F97316] text-black shadow-[0_0_15px_rgba(249,115,22,0.4)]' : 'bg-white/5 text-white/40 border border-white/10 hover:border-white/30'}`}>{type}</button>
                      ))}
                   </div>
                   <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                      {activeCharts.includes('History') && (
                        <div className="glass-panel p-10 rounded-[2.5rem]"><h3 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-10">Training Progression</h3><PerformanceChart result={result} /></div>
                      )}
                      {activeCharts.includes('Metrics') && (
                        <div className="glass-panel p-10 rounded-[2.5rem]"><h3 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-10">Final Fidelity Metrics</h3><MetricsBarChart result={result} /></div>
                      )}
                      {activeCharts.includes('Radar Signature') && (
                        <div className="glass-panel p-10 rounded-[2.5rem] flex flex-col items-center">
                           ...
                           {result ? (
                             <div className="w-full h-80"><ResponsiveContainer><RadarChart data={result.radarMetrics}><PolarGrid stroke="#1e293b"/><PolarAngleAxis dataKey="subject" stroke="#94a3b8" fontSize={10}/><Radar name="Spill Signature" dataKey="value" stroke={COLORS.primary} fill={COLORS.primary} fillOpacity={0.4}/></RadarChart></ResponsiveContainer></div>
                      ...
                      {activeCharts.includes('Inference Pie') && (
                         <div className="glass-panel p-10 rounded-[2.5rem] flex flex-col items-center">
                            ...
                            {result ? (
                              <div className="w-full h-80"><ResponsiveContainer><PieChart><Pie data={result.inferencePath} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="probability"><Cell fill={COLORS.primary}/><Cell fill="#1e293b"/></Pie><Tooltip contentStyle={{backgroundColor: '#0A192F', border: '1px solid #1E293B', borderRadius: '8px'}}/></PieChart></ResponsiveContainer></div>
                            ) : (<div className="flex-1 flex items-center text-white/10 italic">Awaiting mission analysis...</div>)}
                         </div>
                      )}
                   </div>
               ...
               {activeMenu === 'Download Options' && (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                   <div className="glass-panel p-16 rounded-[4rem] text-center">
                     <i className="fa-solid fa-file-pdf text-4xl mb-6 text-slate-500"></i>
                   ...
                   <div className="glass-panel p-16 rounded-[4rem] text-center">
                     <i className="fa-solid fa-images text-4xl mb-6 text-[#F97316]/40"></i>
